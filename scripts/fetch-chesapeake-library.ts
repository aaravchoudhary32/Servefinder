// Manual, one-shot fetcher for the Chesapeake Public Library's public
// volunteer page. Run with `npm run fetch:chesapeake-library`.
//
// The actual fetch/normalize/upsert logic lives in
// lib/ingestion/sources/chesapeakeLibrary.ts, shared with the weekly
// Vercel cron route at app/api/cron/fetch/chesapeake-library/route.ts (dispatched via lib/ingestion/sourceRegistry.ts) —
// this file is just env loading + a CLI-friendly client + printing.

import { existsSync, readFileSync } from "fs";
import { createClient } from "@supabase/supabase-js";
import { runChesapeakeLibraryFetch } from "../lib/ingestion/sources/chesapeakeLibrary";
...
runChesapeakeLibraryFetch(supabase)
  .then((result) => {

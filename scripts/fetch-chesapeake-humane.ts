// Manual, one-shot fetcher for Chesapeake Humane Society's public
// volunteer opportunities page. Run with `npm run fetch:chesapeake-humane`.
//
// The actual fetch/normalize/upsert logic lives in
// lib/ingestion/sources/chesapeakeHumane.ts, shared with the weekly
// Vercel cron route at app/api/cron/fetch/chesapeake-humane/route.ts (dispatched via lib/ingestion/sourceRegistry.ts) —
// this file is just env loading + a CLI-friendly client + printing.

import { existsSync, readFileSync } from "fs";
import { createClient } from "@supabase/supabase-js";
import { runChesapeakeHumaneFetch } from "../lib/ingestion/sources/chesapeakeHumane";
...
runChesapeakeHumaneFetch(supabase)
  .then((result) => {
